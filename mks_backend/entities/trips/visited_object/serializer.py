...
class VisitedObjectSerializer:
    def to_mapped_object(self, work_trip_id: int, contruction_id: int) -> VisitedObject:
    ...
    def convert_list_to_objects(self, work_trip_id: int, contructions: list) -> tuple:
        return tuple(
            self.to_mapped_object(work_trip_id, construction_id) for construction_id in contructions
        )
