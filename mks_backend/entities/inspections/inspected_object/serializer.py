...
class InspectedObjectSerializer:
    def to_mapped_object(self, inspection_id: int, contruction_id: int) -> InspectedObject:
    ...
    def convert_list_to_objects(self, inspection_id: int, contructions: list) -> Tuple[InspectedObject]:
        return tuple(
            self.to_mapped_object(inspection_id, construction_id) for construction_id in contructions
        )
