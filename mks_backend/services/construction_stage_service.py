...
class ConstructionStageService:
    ...
    def update_construction_stage(self, new_construction_stage):
        old_construction_stage = self.repo.get_construction_stage_by_id(new_construction_stage.construction_stages_id)
        if old_construction_stage.code != new_construction_stage.code:
            if self.repo.get_construction_stage_by_code(new_construction_stage.code):
                raise ValueError('Этап строительства с таким кратким наименованием уже существует')
        if old_construction_stage.fullname != new_construction_stage.fullname:
            if self.repo.get_construction_stage_by_fullname(new_construction_stage.fullname):
                raise ValueError('Этап строительства с таким полным наименованием уже существует')
